...
public class EllipsizeTextView extends AppCompatTextView {
    ...
    public EllipsizeTextView(final Context context, final AttributeSet attrs, final int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setText(!TextUtils.isEmpty(mOriginText) ? mOriginText : getText());
    ...
    private void adjustEllipsizeEndText(Layout layout) {
        ...
        final CharSequence restSuffixText = originText.subSequence(originText.length() - mEllipsizeIndex, originText.length());
        ...
        final int suffixWidth = (int) (Layout.getDesiredWidth(ELLIPSIZE_TEXT, getPaint()) + Layout.getDesiredWidth(restSuffixText, getPaint())) + 1;
        mEnableUpdateOriginText = false;
        if (lastLineWidth + suffixWidth > width) {
            final int widthDiff = lastLineWidth + suffixWidth - width;
            final int removedCharacterCount = computeRemovedEllipsizeEndCharacterCount(widthDiff, originText.subSequence(0, mLastCharacterIndex));
     ...
     */
    public void setEllipsizeTextIndex(int ellipsizeIndex) {
